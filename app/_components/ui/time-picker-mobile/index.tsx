...
const TimePickerMobile = ({
  ...
  const reduxSelectedTime = useSelector(
    (state: RootState) => state.jobCreate.selectedTime
  );
  ...
  const handleKeyDown = (event: React.KeyboardEvent) => {
    if (event.key === "ArrowDown") {
      ...
      setFocusedIndex((prevIndex) =>
        prevIndex === null ? 0 : Math.min(prevIndex + 1, times.length - 1)
      );
    } else if (event.key === "ArrowUp") {
      event.preventDefault();
      setIsOpen(true);
      setFocusedIndex((prevIndex) =>
        prevIndex === null ? times.length - 1 : Math.max(prevIndex - 1, 0)
      );
    ...
    <div>
      <div ref={dropdownRef} className="relative w-[123px]">
        <div>
          ...
          <div
            ...
            className="flex justify-center items-center !cursor-pointer relative left-[27px]"
          >
            <Button
              ...
              className={`${
                eventStartTimeError !== ""
                  ...
                  : ""
              } w-full py-2 pl-4 !pr-[10rem]  flex justify-start items-center border ${
                eventStartTimeError === ""
                  ? "border-[#EFEFEF]"
                  : "border-[#FF8F8F]"
              } rounded-md text-left bg-white h-[56px] hover:text-gray-600 hover:bg-gray-50`}
              style={{
                color: "#9F9F9F",
                fontSize: "14px",
                fontWeight: "400",
            ...
            <span className="absolute z-[999] right-[-20px] text-[#9F9F9F]">
          ...
          {isOpen && (
            ...
            >
              ...
              className="bg-white rounded-lg shadow-lg p-4 w-[90%] max-w-sm">
                <ul
                  className="w-full mt-1 py-[4px] px-[8px] rounded-md bg-white min-w-48 overflow-y-auto z-[9999]"
                  style={{
                    height: "320px",
                    scrollbarWidth: "none",
                ...
                >
                  {times.map((time, index) => (
                    <li
                      key={index}
                      ref={reduxSelectedTime === time ? selectedRef : null}
                      ...
                      className={`flex items-center justify-between py-[12px] px-[8px] cursor-pointer !text-[#6B6B6B] font-[400] leading-[20px] text-[14px] 
                                ${
                                  reduxSelectedTime === time
                                    ? "bg-[#F9F7FF] rounded-[4px] !text-[#774DFD]"
                                    : " hover:!bg-[#F9F7FF] !rounded-[4px]"
                                } 
                    ...
                    >
                      <span
                        className={`${
                          reduxSelectedTime === time && "!text-[#774DFD]"
                        }`}
                      >
                        {time}
                      </span>
        ...
        <div className="relative left-[145px] bottom-[56px]">
          <div className="flex mt-2 w-full justify-center ml-[60px] items-center cursor-pointer rounded-[12px] border border-[#EBE6FF] absolute">
            <div
              ...
              className={`w-1/2 py-2 font-[400] flex justify-center items-center leading-[24px] text-[14px] rounded-l-[12px] ${
                reduxAmPm === "AM"
                  ? "bg-[#2C2240] text-[#FFF]"
                  : "bg-white hover:!bg-gray-50 text-[#6B6B6B]"
              }`}
            >
              AM
            </div>
            <div
              ...
              className={`w-1/2 py-2 rounded-r-[12px] font-[400] flex justify-center items-center leading-[24px] text-[14px] ${
                reduxAmPm === "PM"
      ...
      <p
        className={`text-[#C20000] text-[12px] pt-[5px] font-[400] leading-[20px] transition-opacity duration-500 ease-linear ${
          eventStartTimeError !== "" ? "opacity-100" : "opacity-0"
        } ${
          eventStartTimeError !== "" ? "max-h-20" : "max-h-0"
        } overflow-hidden`}
      >
        {eventStartTimeError}
      </p>
