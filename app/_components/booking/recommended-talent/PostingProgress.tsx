...
const PostingProgress = () => {
    ...
    const apiCalled = useRef(false); // Ref to ensure the API is called only once
    ...
    const createJobApi = async () => {
        if (apiCalled.current) return; // Return early if the API was already called

        apiCalled.current = true; // Set flag to prevent further calls

        const endpoint = "/job";
        const dateOnly = jobCreateState.selectedDate ? new Date(Date.UTC(new Date(jobCreateState?.selectedDate)?.getFullYear(), new Date(jobCreateState?.selectedDate)?.getMonth(), new Date(jobCreateState?.selectedDate)?.getDate())) : null;
        const selectedDate = dateOnly ? dateOnly.toISOString().split('T')[0] : null;
        const startTime = convertTo24Hour(jobCreateState.selectedTime, jobCreateState.amPm);
        const endTime = addHoursToTime(startTime, parseInt(jobCreateState.selectedHours));
