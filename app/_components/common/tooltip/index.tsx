...
const Tooltip: React.FC<TooltipProps> = ({ children, content }) => {
  ...
  return (
    <div className="relative inline-block cursor-pointer z-20" ref={containerRef}>
      <div 
      ...
      {isOpen && (
        <div
          ref={tooltipRef}
          className={`absolute !left-[39px] ${position === 'top' ? 'bottom-full mb-2' : 'top-full mt-2'} left-1/2 transform -translate-x-1/2 bg-[#FFF] text-black text-sm rounded shadow-lg z-40`}
        >
          {content}
          {position !== "bottom" && (
            <span>
